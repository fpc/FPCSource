        ...
        case nodetype of
          ...
          lten,gten:
            begin
              ...
              thlcgwasm(ctx.hlcg).a_load_loc_stack(ctx.CurrAsmList,right.resultdef,right.location);
              thlcgwasm(ctx.hlcg).a_load_loc_stack(ctx.CurrAsmList,left.resultdef,left.location);
              thlcgwasm(ctx.hlcg).a_op_stack(ctx.CurrAsmList,OP_AND,resultdef);
              thlcgwasm(ctx.hlcg).a_load_loc_stack(ctx.CurrAsmList,left.resultdef,left.location);
              thlcgwasm(ctx.hlcg).a_cmp_stack_stack(ctx.CurrAsmList,resultdef,OC_EQ);
              set_result_location_reg(ctx);
              thlcgwasm(ctx.hlcg).a_load_stack_loc(ctx.CurrAsmList,resultdef,location);
            end;
          else
    ...
    procedure twasmaddnode.second_generic_compare(unsigned: boolean;ctx:tpassgeneratecodecontext);
      ...
      begin
        ...
        if left.location.loc in [LOC_REGISTER,LOC_CREGISTER] then
          thlcgwasm(ctx.hlcg).a_cmp_loc_reg_stack(ctx.CurrAsmList,left.resultdef,cmpop,right.location,left.location.register)
        else case right.location.loc of
          LOC_REGISTER,LOC_CREGISTER:
            thlcgwasm(ctx.hlcg).a_cmp_reg_loc_stack(ctx.CurrAsmList,left.resultdef,cmpop,right.location.register,left.location);
          LOC_REFERENCE,LOC_CREFERENCE:
            thlcgwasm(ctx.hlcg).a_cmp_ref_loc_stack(ctx.CurrAsmList,left.resultdef,cmpop,right.location.reference,left.location);
          LOC_CONSTANT:
            thlcgwasm(ctx.hlcg).a_cmp_const_loc_stack(ctx.CurrAsmList,left.resultdef,cmpop,right.location.value,left.location);
          else
            begin
