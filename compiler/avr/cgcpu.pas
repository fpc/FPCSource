        ...
        if CPUAVR_16_REGS in compiler.target.cpu_capabilities[compiler.globals.current_settings.cputype] then
          ...
          rg[R_INTREGISTER]:=trgintcpu.create(R_INTREGISTER,R_SUBWHOLE,
              [RS_R18,RS_R19,RS_R20,RS_R21,RS_R22,RS_R23,RS_R24,RS_R25,
               ...
               RS_R10,RS_R11,RS_R12,RS_R13,RS_R14,RS_R15,RS_R16,RS_R17],first_int_imreg,[],compiler);
      end;
     ...
     procedure tcgavr.a_op_const_reg_reg_internal(list: TAsmList; op: TOpCg; size: tcgsize; a: tcgint; src,srchi,dst,dsthi: tregister);
       ...
       begin
         if (op in [OP_MUL,OP_IMUL]) and (size in [OS_16,OS_S16]) and (a in [2,4,8]) then
           ...
           { a=0 get eliminated later by toptimize_op_const }
           (a>0)  then
           begin
             ...
             if b2 > 0 then
               begin
                 ...
                 if not(cs_opt_size in compiler.globals.current_settings.optimizerswitches) or
...
{$endif}
    ...
    begin
      result := (not(compiler.globals.current_settings.cputype in [cpu_avrxmega3,cpu_avrtiny]) and (addr>31)) or
                ((compiler.globals.current_settings.cputype in [cpu_avrxmega3,cpu_avrtiny]) and (addr>=0)) and
                (addr<cpuinfo.embedded_controllers[compiler.globals.current_settings.controllertype].srambase);
    end;
       ...
       begin
         ...
         if not conv_done then
           begin
             ...
             if (compiler.globals.current_settings.cputype <> cpu_avrxmega3) and
          ...
          begin
            if (CPUAVR_HAS_MUL in compiler.target.cpu_capabilities[compiler.globals.current_settings.cputype]) and
              ...
              begin
                ...
                a_load_reg_reg(list,OS_8,OS_8,NR_R24,dst);
          ...
          begin
            if (CPUAVR_HAS_MUL in compiler.target.cpu_capabilities[compiler.globals.current_settings.cputype]) and
              ...
              begin
                ...
                a_load_reg_reg(list,OS_8,OS_8,NR_R24,dst);
                a_reg_dealloc(list,NR_R24);
                a_load_reg_reg(list,OS_8,OS_8,NR_R25,GetNextReg(dst));
          ...
          begin
            ...
            if compiler.globals.current_settings.cputype=cpu_avr1 then
              ...
              begin
                ...
                { Save SREG }
                getcpuregister(list,GetDefaultTmpReg);
                list.concat(taicpu.op_reg_const(A_IN, GetDefaultTmpReg, $3F));
                list.concat(taicpu.op_reg(A_PUSH, GetDefaultTmpReg));
                ungetcpuregister(list,GetDefaultTmpReg);
              end;
      ...
      begin
        if len>16 then
          begin
            ...
            ungetcpuregister(list,NR_R26);
            ungetcpuregister(list,NR_R27);
            ungetcpuregister(list,NR_R30);
            ungetcpuregister(list,NR_R31);
            { keep registers alive }
            a_reg_sync(list,countreg);
          end
        else
          begin
            ...
            if (len = 2) and DestQuickRef and (compiler.globals.current_settings.cputype <> cpu_avrxmega3) and
              ...
              begin
                ...
                if SrcQuickRef and addr_is_io_register(srcref.offset) then
                  ...
                  begin
                    ...
                    if compiler.globals.current_settings.cputype = cpu_avrtiny then
                      begin
                    ...
                    if not(SrcQuickRef) and (compiler.globals.current_settings.cputype <> cpu_avrtiny) then
                      srcref.addressmode:=AM_POSTINCREMENT
                    else if compiler.globals.current_settings.cputype = cpu_avrtiny then
                      srcref.addressmode:=AM_PREDECREMENT
                    else
                      srcref.addressmode:=AM_UNCHANGED;
