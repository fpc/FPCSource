...
{$i fpcdefs.inc}
  ...
  implementation
      ...
      begin
        ...
        if (abs(spilltemp.offset)>63) or (CPUAVR_16_REGS in compiler.target.cpu_capabilities[compiler.globals.current_settings.cputype]) then
          begin
            ...
            if (CPUAVR_HAS_ADIW in compiler.target.cpu_capabilities[compiler.globals.current_settings.cputype]) and (ofs>0) and (ofs<=126) then
              begin
    ...
    function trgcpu.do_spill_replace(list:TAsmList;instr:tai_cpu_abstract_sym;orgreg:tsuperregister;const spilltemp:treference):boolean;
      begin
        result:=false;
        if not(spilltemp.offset in [0..63]) or (CPUAVR_16_REGS in compiler.target.cpu_capabilities[compiler.globals.current_settings.cputype]) then
          exit;
