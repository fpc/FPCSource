...
{$endif i8086}
          ...
          constructor Create_type_name(AsmData: TAsmData;_typ:taiconst_type;const name:string;symclass: TAsmSymbolClass;_symtyp:Tasmsymtype;ofs:asizeint);
 ...
 ****************************************************************************}
    ...
    constructor tai_datablock.Create(AsmData: TAsmData; const _name : string;_size : asizeint; def: tdef; _typ:Tasmsymtype);

      begin
         inherited Create;
         typ:=ait_datablock;
         sym:=AsmData.DefineAsmSymbol(_name,AB_LOCAL,_typ,def);
         { keep things aligned }
         if _size<=0 then
           _size:=sizeof(aint);
         size:=_size;
    ...
    constructor tai_datablock.Create_hidden(AsmData: TAsmData; const _name: string; _size: asizeint; def: tdef; _typ:Tasmsymtype);
      var
        compiler: TCompilerBase absolute current_compiler;  { TODO: fix node compiler reference!!! }
      begin
        if tf_supports_hidden_symbols in compiler.target.info.flags then
          begin
            inherited Create;
            typ:=ait_datablock;
            sym:=AsmData.DefineAsmSymbol(_name,AB_PRIVATE_EXTERN,_typ,def);
            { keep things aligned }
            if _size<=0 then
              _size:=sizeof(aint);
            size:=_size;
...
{$ifdef i8086}
    ...
    constructor tai_const.Createname_near(AsmData: TAsmData;const name:string;_symtyp:Tasmsymtype;ofs:asizeint);
      begin
        self.Createname(AsmData,name,_symtyp,ofs);
        consttype:=aitconst_ptr;
      end;


    constructor tai_const.Createname_far(AsmData: TAsmData;const name:string;_symtyp:Tasmsymtype;ofs:asizeint);
      begin
        self.Createname(AsmData,name,_symtyp,ofs);
        consttype:=aitconst_farptr;
      end;
...
{$endif i8086}
    ...
    constructor tai_const.Createname_rel(AsmData: TAsmData;const name,endname:string);
      begin
         self.create_sym_offset(AsmData.RefAsmSymbol(name,AT_NONE),0);
         endsym:=AsmData.RefAsmSymbol(endname,AT_NONE)
      end;
    ...
    constructor tai_const.Create_type_name(AsmData: TAsmData;_typ:taiconst_type;const name:string;_symtyp:Tasmsymtype;ofs:asizeint);
      begin
         self.create_sym_offset(AsmData.RefAsmSymbol(name,_symtyp),ofs);
         consttype:=_typ;
      end;
    ...
    constructor tai_const.Create_type_name(AsmData: TAsmData;_typ:taiconst_type;const name:string;symclass: TAsmSymbolClass;_symtyp:Tasmsymtype;ofs:asizeint);
      begin
        self.create_sym_offset(AsmData.RefAsmSymbolByClass(symclass,name,_symtyp),ofs);
        consttype:=_typ;
      end;
