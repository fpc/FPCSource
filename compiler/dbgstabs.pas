      ...
      begin
        ...
        case def.stringtype of
         ...
         st_ansistring:
           begin
             { looks like a pchar }
             ss:='*'+def_stab_number(compiler.deftypes.cansichartype);
           end;
         st_unicodestring,
         st_widestring:
           begin
...
****************************************************************************}
      ...
      begin
        ...
        if not(compiler.target.info.system in systems_darwin) then
          begin
            ...
            AsmData.asmlists[al_stabs].concat(tai_symbol.Createname_global(AsmData,make_mangledname('DEBUGINFO',compiler.current_module.localsymtable,''),AT_METADATA,0,compiler.deftypes.voidpointertype));
          end
        else
                    ...
                    if assigned(infile) then
                      begin
                        ...
                        if not(ds_stabs_abs_include_files in compiler.globals.current_settings.debugswitches) or
                          ...
                          list.insertbefore(Tai_stab.Create_str(stabsdir,'"'+compiler.CFileUtl.BsToSlash(compiler.CFileUtl.FixPath(infile.path,false))+compiler.CFileUtl.FixFileName(infile.name)+'",'+tostr(stabs_n_includefile)+
                                            ',0,0,'+hlabel.name),hp)
                        else
                          list.insertbefore(Tai_stab.Create_str(stabsdir,'"'+compiler.CFileUtl.BsToSlash(compiler.CFileUtl.FixPath(compiler.CFileUtl.getcurrentdir,false)+compiler.CFileUtl.FixPath(infile.path,false))+compiler.CFileUtl.FixFileName(infile.name)+'",'+tostr(stabs_n_includefile)+
                                            ',0,0,'+hlabel.name),hp);
    ...
    procedure TDebugInfoStabs.insertmoduleinfo;
      ...
      begin
        ...
        if not(compiler.target.info.system in systems_darwin) then
          AsmData.asmlists[al_start].concat(tai_symbol.Createname_global(AsmData,make_mangledname('DEBUGSTART',compiler.current_module.localsymtable,''),AT_METADATA,0,compiler.deftypes.voidpointertype));
{$ifdef MIPS}
...
{$endif MIPS}
        AsmData.asmlists[al_start].concat(Tai_stab.Create_str(stabsdir,'"'+compiler.CFileUtl.BsToSlash(compiler.CFileUtl.FixPath(compiler.CFileUtl.getcurrentdir,false))+'",'+
          base_stabs_str(stabs_n_sourcefile,'0','0',hlabel.name)));
        AsmData.asmlists[al_start].concat(Tai_stab.Create_str(stabsdir,'"'+compiler.CFileUtl.BsToSlash(compiler.CFileUtl.FixPath(infile.path,false))+compiler.CFileUtl.FixFileName(infile.name)+'",'+
          base_stabs_str(stabs_n_sourcefile,'0','0',hlabel.name)));
        hlabel.increfs;
        ...
        if not(compiler.target.info.system in systems_darwin) then
          AsmData.asmlists[al_end].concat(tai_symbol.Createname_global(AsmData,make_mangledname('DEBUGEND',compiler.current_module.localsymtable,''),AT_METADATA,0,compiler.deftypes.voidpointertype));
