...
*)
    ...
    function tjvminlinenode.first_setlength_array: tnode;
      ...
      begin
        ...
        { prepend deepcopy }
        newparas:=compiler.ccallparanode(compiler.cordconstnode(0,compiler.deftypes.pasbool1type,false),newparas);
      ...
      begin
        if is_dynamic_array(left.resultdef) or
          ...
          begin
            ...
            secondpass(left,ctx);
            thlcgjvm(ctx.hlcg).g_getarraylen(ctx.CurrAsmList,left.location);
         ...
         case left.location.size of
           OS_F32:
             begin
               ...
               ctx.CurrAsmList.concat(taicpu.op_none(a_fmul));
               thlcgjvm(ctx.hlcg).decstack(ctx.CurrAsmList,1);
             end;
           OS_F64:
             begin
               ctx.CurrAsmList.concat(taicpu.op_none(a_dup2));
               thlcgjvm(ctx.hlcg).incstack(ctx.CurrAsmList,2);
               ctx.CurrAsmList.concat(taicpu.op_none(a_dmul));
    ...
    procedure tjvminlinenode.second_trunc_real(ctx:tpassgeneratecodecontext);
      begin
         ...
         case left.location.size of
           OS_F32:
             begin
               ctx.CurrAsmList.concat(taicpu.op_none(a_f2l));
           ...
           OS_F64:
             begin
               ...
               ctx.CurrAsmList.concat(taicpu.op_none(a_d2l));
             end;
           else
    ...
    procedure tjvminlinenode.second_new(ctx:tpassgeneratecodecontext);
      ...
      begin
        ...
        repeat
          ...
          thlcgjvm(ctx.hlcg).a_load_loc_stack(ctx.CurrAsmList,hp.left.resultdef,hp.left.location);
          hp:=tcallparanode(hp.right);
        until not assigned(hp);
        { create the array }
        thlcgjvm(ctx.hlcg).g_newarray(ctx.CurrAsmList,arr.resultdef,paracount);
        location_reset(location,LOC_REGISTER,OS_ADDR);
        location.register:=ctx.hlcg.getaddressregister(ctx.CurrAsmList,resultdef);
        thlcgjvm(ctx.hlcg).a_load_stack_reg(ctx.CurrAsmList,arr.resultdef,location.register);
      end;
    ...
    procedure tjvminlinenode.second_setlength(ctx:tpassgeneratecodecontext);
      ...
      begin
        ...
        { can't directly load from stack to destination, because if target is
          ...
          value }
        tmpreg:=ctx.hlcg.getaddressregister(ctx.CurrAsmList,target.resultdef);
        if is_wide_or_unicode_string(target.resultdef) then
          begin
            emptystr:=#0;
            ctx.CurrAsmList.concat(taicpu.op_string(a_ldc,0,@emptystr));
            thlcgjvm(ctx.hlcg).incstack(ctx.CurrAsmList,1);
          end
        else if is_ansistring(target.resultdef) then
          thlcgjvm(ctx.hlcg).a_load_const_stack(ctx.CurrAsmList,compiler.deftypes.java_jlobject,0,R_ADDRESSREGISTER)
        else if is_dynamic_array(target.resultdef) then
          begin
            thlcgjvm(ctx.hlcg).a_load_const_stack(ctx.CurrAsmList,compiler.deftypes.s32inttype,0,R_INTREGISTER);
            thlcgjvm(ctx.hlcg).g_newarray(ctx.CurrAsmList,target.resultdef,1);
          end
        else
          internalerror(2011031401);
        thlcgjvm(ctx.hlcg).a_load_stack_reg(ctx.CurrAsmList,target.resultdef,tmpreg);
        thlcgjvm(ctx.hlcg).a_load_reg_loc(ctx.CurrAsmList,target.resultdef,target.resultdef,tmpreg,target.location);
      end;
