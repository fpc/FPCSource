...
function tjvmassignmentnode.direct_shortstring_assignment: boolean;
  begin
    if maybe_find_real_class_definition(compiler.symtablestack,right.resultdef,false)=compiler.deftypes.java_jlstring then
  ...
  begin
    ...
    if (target.nodetype=vecn) and
      ...
      begin
        ...
        inserttypeconv_explicit(tvecnode(target).left,compiler.deftypes.java_shortstring,compiler);
        psym:=search_struct_member(tabstractrecorddef(compiler.deftypes.java_shortstring),'SETCHAR');
