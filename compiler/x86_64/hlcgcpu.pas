      ...
      if (po_virtualmethod in procdef.procoptions) and
        ...
        begin
          ...
          if x86_64_use_ms_abi(procdef.proccalloption,compiler.target) then
            reference_reset_base(href,compiler.deftypes.voidpointertype,NR_RCX,0,ctempposinvalid,sizeof(pint),[])
          else
            reference_reset_base(href,compiler.deftypes.voidpointertype,NR_RDI,0,ctempposinvalid,sizeof(pint),[]);
          cg.a_load_ref_reg(list,OS_ADDR,OS_ADDR,href,NR_RAX);
          { jmp *vmtoffs(%eax) ; method offs }
          reference_reset_base(href,compiler.deftypes.voidpointertype,NR_RAX,tobjectdef(procdef.struct).vmtmethodoffset(procdef.extnumber),ctempposinvalid,sizeof(pint),[]);
          list.concat(taicpu.op_ref(A_JMP,S_Q,href));
        end
      else
        begin
          sym:=list.AsmData.RefAsmSymbol(procdef.mangledname,AT_FUNCTION);
