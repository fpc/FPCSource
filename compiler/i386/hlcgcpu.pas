...
implementation
  ...
  begin
    ...
    need_got_load:=not (compiler.target.info.system in systems_darwin) and
  ...
  procedure thlcgcpu.g_exception_reason_save(list: TAsmList; fromsize, tosize: tdef; reg: tregister; const href: treference);
    begin
      if not(paramanager.use_fixed_stack) and not(tf_use_psabieh in compiler.target.info.flags) then
