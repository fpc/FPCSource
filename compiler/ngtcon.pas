...
interface
    ...
    type
      ttypedconstbuilder = class
       ...
       public
        constructor create(sym: tstaticvarsym; acompiler: TCompilerBase);
      end;
      ...
      tnodetreetypedconstbuilder = class(ttypedconstbuilder)
       protected
        ...
        procedure tc_emit_stringdef(def: tstringdef; var node: tnode);override;
       public
        constructor create(sym: tstaticvarsym; previnit: tnode; acompiler: TCompilerBase);virtual;
        destructor destroy;override;
...
*****************************************************************************}
    ...
    constructor ttypedconstbuilder.create(sym: tstaticvarsym; acompiler: TCompilerBase);
      begin
        FCompiler:=acompiler;
        tcsym:=sym;
      end;
        ...
        if (def=compiler.deftypes.rec_tguid) and (current_scanner.token=_ID) then
          begin
            ...
            if n.nodetype=stringconstn then
              ...
              begin
                inserttypeconv(n,compiler.deftypes.rec_tguid,compiler);
      ...
      begin
        ...
        { array const between brackets }
          ...
          begin
            ...
            for i:=def.lowrange to def.highrange-1 do
              begin
                basenode:=compiler.cvecnode(orgbase.getcopy,compiler.ctypeconvnode_explicit(genintconstnode(i,compiler),tarraydef(def).rangedef));
                read_typed_const_data(def.elementdef);
                if current_scanner.token=_RKLAMMER then
                  begin
                    compiler.verbose.Message1(parser_e_more_array_elements_expected,tostr(def.highrange-i));
