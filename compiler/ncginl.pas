    ...
    procedure tcginlinenode.second_high(ctx:tpassgeneratecodecontext);
      ...
      begin
        ...
        { volatility of the dyn. array refers to the volatility of the
          string pointer, not of the string data }
        ctx.hlcg.reference_reset_base(href,left.resultdef,left.location.register,-compiler.deftypes.sizesinttype.size,ctempposinvalid,compiler.deftypes.ossinttype.alignment,[]);
        { if the string pointer is nil, the length is 0 -> reuse the register
          that originally held the string pointer for the length, so that we
          can keep the original nil/0 as length in that case }
        hregister:=ctx.cg.makeregsize(ctx.CurrAsmList,left.location.register,def_cgsize(resultdef));
        ctx.hlcg.a_load_ref_reg(ctx.CurrAsmList,compiler.deftypes.sizesinttype,resultdef,href,hregister);
        ctx.hlcg.a_jmp_always(ctx.CurrAsmList,donelab);

        ctx.cg.a_label(ctx.CurrAsmList,nillab);
        ctx.hlcg.a_op_const_reg(ctx.CurrAsmList,OP_SUB,resultdef,1,hregister);
...
{$if not defined(cpu64bitalu) and not defined(cpuhighleveltarget)}
        if def_cgsize(resultdef) in [OS_64,OS_S64] then
          begin
            ...
            ctx.cg64.a_op64_const_reg_reg(ctx.CurrAsmList,cgop,def_cgsize(resultdef),1,left.location.register64,location.register64);
          end
        else
{$endif not cpu64bitalu and not cpuhighleveltarget}
          begin
            location.register:=ctx.hlcg.getregisterfordef(ctx.CurrAsmList,resultdef);
            ctx.hlcg.a_op_const_reg_reg(ctx.CurrAsmList,cgop,resultdef,1,left.location.register,location.register);
          end;
      end;
              ...
              if is_constintnode(tcallparanode(tcallparanode(left).right).left) then
                ...
                begin
                  if not(tcallparanode(tcallparanode(left).right).left.location.loc in [LOC_REGISTER,LOC_CREGISTER,LOC_REFERENCE,LOC_CREFERENCE]) or (addvalue>1) or
                    ...
                    begin
                      ctx.hlcg.location_force_reg(ctx.CurrAsmList,tcallparanode(tcallparanode(left).right).left.location,tcallparanode(tcallparanode(left).right).left.resultdef,second_incdec_tempregdef,addvalue<=1);
...
{$endif not defined(cpu64bitalu) and not defined(cpuhighleveltarget)}
                      ...
                      if addvalue>1 then
                        ctx.hlcg.a_op_const_reg(ctx.CurrAsmList,OP_IMUL,left.resultdef,addvalue.svalue,hregister);
                    end
...
{$if not defined(cpu64bitalu) and not defined(cpuhighleveltarget)}
              if def_cgsize(left.resultdef) in [OS_64,OS_S64] then
                ...
                ctx.cg64.a_op64_const_loc(ctx.CurrAsmList,addsubop[inlinenumber],def_cgsize(left.resultdef),addvalue.svalue,tcallparanode(left).left.location)
              else
{$endif not cpu64bitalu and not cpuhighleveltarget}
                ctx.hlcg.a_op_const_loc(ctx.CurrAsmList,addsubop[inlinenumber],left.resultdef,
{$ifdef cpu64bitalu}
               ...
               if def_cgsize(left.resultdef) in [OS_64,OS_S64] then
                 case tcallparanode(tcallparanode(left).right).left.location.loc of
                   LOC_REFERENCE,LOC_CREFERENCE:
                     ctx.cg64.a_op64_ref_loc(ctx.CurrAsmList,addsubop[inlinenumber],def_cgsize(left.resultdef),
                       tcallparanode(tcallparanode(left).right).left.location.reference,tcallparanode(left).left.location);
                   LOC_REGISTER,LOC_CREGISTER:
                     ctx.cg64.a_op64_reg_loc(ctx.CurrAsmList,addsubop[inlinenumber],def_cgsize(left.resultdef),
...
{$endif not cpu64bitalu and not cpuhighleveltarget}
                 case tcallparanode(tcallparanode(left).right).left.location.loc of
                   LOC_REFERENCE,LOC_CREFERENCE:
                     ctx.hlcg.a_op_ref_loc(ctx.CurrAsmList,addsubop[inlinenumber],left.resultdef,
                       tcallparanode(tcallparanode(left).right).left.location.reference,tcallparanode(left).left.location);
                   LOC_REGISTER,LOC_CREGISTER:
                     ctx.hlcg.a_op_reg_loc(ctx.CurrAsmList,addsubop[inlinenumber],left.resultdef,
...
//              ctx.cg.g_overflowcheck(ctx.CurrAsmList,tcallparanode(left).left.location,tcallparanode(left).resultdef);
//              ctx.cg.g_rangecheck(ctx.CurrAsmList,tcallparanode(left).left.location,tcallparanode(left).left.resultdef,
          ...
          if is_constintnode(tcallparanode(left).left) then
            ...
            begin
              ...
              if inlinenumber in [in_sar_assign_x_y,in_shl_assign_x_y,in_shr_assign_x_y,in_rol_assign_x_y,in_ror_assign_x_y] then
                ctx.hlcg.location_force_reg(ctx.CurrAsmList,tcallparanode(left).left.location,tcallparanode(left).left.resultdef,tcallparanode(left).left.resultdef,true)
              else
                ctx.hlcg.location_force_reg(ctx.CurrAsmList,tcallparanode(left).left.location,tcallparanode(left).left.resultdef,tcallparanode(left).right.resultdef,true);
...
{$if not defined(cpu64bitalu) and not defined(cpuhighleveltarget)}
              if (def_cgsize(tcallparanode(left).right.resultdef) in [OS_64,OS_S64]) and
                ...
                ctx.cg64.a_op64_const_loc(ctx.CurrAsmList,andorxorop[inlinenumber],def_cgsize(tcallparanode(left).right.resultdef),maskvalue.svalue,tcallparanode(tcallparanode(left).right).left.location)
              else
{$endif not cpu64bitalu and not cpuhighleveltarget}
                ctx.hlcg.a_op_const_loc(ctx.CurrAsmList,andorxorop[inlinenumber],tcallparanode(left).right.resultdef,
{$ifdef cpu64bitalu}
...
{$if not defined(cpu64bitalu) and not defined(cpuhighleveltarget)}
               if (def_cgsize(tcallparanode(left).right.resultdef) in [OS_64,OS_S64]) and
                  (tcallparanode(tcallparanode(left).right).left.location.loc in [LOC_REFERENCE,LOC_CREFERENCE,LOC_REGISTER,LOC_CREGISTER]) then
                 ctx.cg64.a_op64_reg_loc(ctx.CurrAsmList,andorxorop[inlinenumber],def_cgsize(tcallparanode(left).right.resultdef),
                   joinreg64(hregister,hregisterhi),tcallparanode(tcallparanode(left).right).left.location)
               else
{$endif not cpu64bitalu and not cpuhighleveltarget}
                 ctx.hlcg.a_op_reg_loc(ctx.CurrAsmList,andorxorop[inlinenumber],tcallparanode(left).right.resultdef,
...
{$if not defined(cpu64bitalu) } // and not defined(cpuhighleveltarget)}
          if (def_cgsize(left.resultdef) in [OS_64,OS_S64]) and (left.location.loc in [LOC_REGISTER,LOC_CREGISTER,LOC_REFERENCE,LOC_CREFERENCE]) then
            ctx.cg64.a_op64_loc(ctx.CurrAsmList,negnotop[inlinenumber],def_cgsize(left.resultdef),left.location)
          else
{$endif not cpu64bitalu}
            ctx.hlcg.a_op_loc(ctx.CurrAsmList,negnotop[inlinenumber],left.resultdef,left.location);
        end;
      ...
      procedure tcginlinenode.second_IncludeExclude(ctx:tpassgeneratecodecontext);
        ...
        begin
          ...
          if elepara.location.loc=LOC_CONSTANT then
            begin
              ctx.hlcg.a_bit_set_const_loc(ctx.CurrAsmList,(inlinenumber=in_include_x_y),
                setpara.resultdef,elepara.location.value-tsetdef(setpara.resultdef).setbase,setpara.location);
            end
          else
            begin
              ctx.hlcg.location_force_reg(ctx.CurrAsmList,elepara.location,elepara.resultdef,compiler.deftypes.u32inttype,true);
              register_maybe_adjust_setbase(ctx.hlcg,ctx.CurrAsmList,compiler.deftypes.u32inttype,elepara.location,tsetdef(setpara.resultdef).setbase);
              ctx.hlcg.a_bit_set_reg_loc(ctx.CurrAsmList,(inlinenumber=in_include_x_y),
...
{$if not(defined(cpu64bitalu)) and not defined(cpuhighleveltarget)}
        if is_64bitint(left.resultdef) then
          begin
            ...
            ctx.cg.a_op_const_reg(ctx.CurrAsmList,OP_SAR,OS_32,31,left.location.register64.reghi);
            tempreg64.reghi:=left.location.register64.reghi;
            tempreg64.reglo:=left.location.register64.reghi;
            ctx.cg64.a_op64_reg_reg(ctx.CurrAsmList,OP_XOR,def_cgsize(resultdef),tempreg64,location.register64);
            if cs_check_overflow in compiler.globals.current_settings.localswitches then
              begin
                ctx.cg64.a_op64_reg_reg_reg_checkoverflow(ctx.CurrAsmList,OP_SUB,def_cgsize(resultdef),tempreg64,location.register64,location.register64,true,ovloc);
                ctx.hlcg.g_overflowcheck_loc(ctx.CurrAsmList,Location,resultdef,ovloc);
              end
            else
              ctx.cg64.a_op64_reg_reg_reg(ctx.CurrAsmList,OP_SUB,def_cgsize(resultdef),tempreg64,location.register64,location.register64);
          end
        else
{$endif not(defined(cpu64bitalu)) and not defined(cpuhighleveltarget)}
          begin
            ...
            tempreg1:=ctx.hlcg.getintregister(ctx.CurrAsmList,left.resultdef);
            tempreg2:=ctx.hlcg.getintregister(ctx.CurrAsmList,left.resultdef);

            ctx.hlcg.a_op_const_reg_reg(ctx.CurrAsmList,OP_SAR,left.resultdef,left.resultdef.size*8-1,left.location.register,tempreg1);
            ctx.hlcg.a_op_reg_reg_reg(ctx.CurrAsmList,OP_XOR,left.resultdef,left.location.register,tempreg1,tempreg2);

            if cs_check_overflow in compiler.globals.current_settings.localswitches then
              begin
                ctx.hlcg.a_op_reg_reg_reg_checkoverflow(ctx.CurrAsmList,OP_SUB,resultdef,tempreg1,tempreg2,location.register,true,ovloc);
                ctx.hlcg.g_overflowcheck_loc(ctx.CurrAsmList,Location,resultdef,ovloc);
              end
            else
              ctx.hlcg.a_op_reg_reg_reg(ctx.CurrAsmList,OP_SUB,resultdef,tempreg1,tempreg2,location.register);
          end;
      end;
      ...
      if left<>nil then
        begin
          ...
          if left.location.loc=LOC_CONSTANT then
            ...
            begin
              ctx.hlcg.location_force_reg(ctx.CurrAsmList,left.location,left.resultdef,compiler.deftypes.voidpointertype,false);
      ...
      begin
        if compiler.current_procinfo.framepointer=NR_STACK_POINTER_REG then
          begin
            ...
            ctx.cg.a_load_ref_reg(ctx.CurrAsmList,OS_ADDR,OS_ADDR,frame_ref,location.register);
          end
        else
          begin
            location_reset(location,LOC_REGISTER,OS_ADDR);
    ...
    procedure tcginlinenode.second_rox_sar(ctx:tpassgeneratecodecontext);
      ...
      begin
        ...
        if not(op1.location.loc in [LOC_REGISTER,LOC_CREGISTER]) then
          ctx.hlcg.location_force_reg(ctx.CurrAsmList,op1.location,op1.resultdef,resultdef,true);
...
{$if not defined(cpu64bitalu) and not defined(cpuhighleveltarget)}
               if def_cgsize(resultdef) in [OS_64,OS_S64] then
                 ctx.cg64.a_op64_const_reg_reg(ctx.CurrAsmList,op,def_cgsize(resultdef),
                   tordconstnode(op2).value.uvalue and (resultdef.size*8-1),
                   op1.location.register64, location.register64)
               else
{$endif not cpu64bitalu and not cpuhighleveltarget}
                 ctx.hlcg.a_op_const_reg_reg(ctx.CurrAsmList,op,resultdef,
                   tordconstnode(op2).value.uvalue and (resultdef.size*8-1),
                 ...
                 if def_cgsize(resultdef) in [OS_64,OS_S64] then
                   begin
                     if not(op2.location.loc in [LOC_REGISTER,LOC_CREGISTER]) or
                       ...
                       ctx.hlcg.location_force_reg(ctx.CurrAsmList,op2.location,
                                               op2.resultdef,compiler.deftypes.alusinttype,true);
                     ctx.cg64.a_op64_reg_reg_reg(ctx.CurrAsmList,op,def_cgsize(resultdef),
...
{$endif not cpu64bitalu and not cpuhighleveltarget}
                   begin
                     ...
                     ctx.hlcg.a_op_reg_reg_reg(ctx.CurrAsmList,op,resultdef,
...
{$if not defined(cpu64bitalu) and not defined(cpuhighleveltarget)}
          if def_cgsize(resultdef) in [OS_64,OS_S64] then
            ctx.cg64.a_op64_const_reg_reg(ctx.CurrAsmList,op,def_cgsize(resultdef),1,
                                      op1.location.register64,location.register64)
          else
{$endif not cpu64bitalu and not cpuhighleveltarget}
            ctx.hlcg.a_op_const_reg_reg(ctx.CurrAsmList,op,resultdef,1,
                                    op1.location.register,location.register);
      end;
    ...
    begin
      ...
      ctx.cg.a_bit_scan_reg_reg(ctx.CurrAsmList,inlinenumber=in_bsr_x,node_not_zero(left),opsize,location.size,left.location.register,location.register);
    end;
