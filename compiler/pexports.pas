                     ...
                     begin
                       ...
                       if pt.nodetype=ordconstn then
                        if (Tordconstnode(pt).value<int64(low(index))) or
                          ...
                          begin
                            index:=0;
                            compiler.verbose.Message3(type_e_range_check_error_bounds,tostr(Tordconstnode(pt).value),tostr(low(index)),tostr(high(index)))
                          end
                        else
                    ...
                    procsym:
                      begin
                       ...
                       if (options*[eo_name,eo_index]=[]) and
                         ...
                         compiler.exportlib.exportallprocsymnames(parser.AsmData,tprocsym(srsym),options)
                       else
                         begin
                           ...
                           compiler.exportlib.exportprocsym(parser.AsmData,srsym,hpname,index,options);
                         end
                      end;
                    staticvarsym:
                      begin
                        if not (eo_name in options) then
                          ...
                          if (vo_has_mangledname in tstaticvarsym(srsym).varoptions) then
                            ...
                            hpname:=orgs;
                        compiler.exportlib.exportvarsym(parser.AsmData,srsym,hpname,index,options);
                      end;
                    typesym:
                      begin
                        case ttypesym(srsym).typedef.typ of
                          objectdef:
                            case tobjectdef(ttypesym(srsym).typedef).objecttype of
                              odt_objcclass:
                                compiler.objcutil.exportobjcclass(parser.AsmData,tobjectdef(ttypesym(srsym).typedef));
                              else
