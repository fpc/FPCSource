      ...
      begin
        ...
        if assigned(left) then
          begin
            ...
            left:=acompiler.ctypeconvnode(left,compiler.current_procinfo.procdef.returndef);
            left:=compiler.cassignmentnode(
              compiler.cloadnode(compiler.current_procinfo.procdef.funcretsym,compiler.current_procinfo.procdef.funcretsym.owner),
              left);
          end;
      end;
    ...
    function texitnode.pass_typecheck:tnode;
      ...
      begin
        ...
        if (compiler.target.info.system in systems_fpnestedstruct) and
           ...
           compiler.current_procinfo.procdef.get_funcretsym_info(ressym,resdef) and
        ...
        if not(assigned(labelnode)) then
          begin
            ...
            if assigned(labelsym.code) and
              ((assigned(labelsym.owner) and (compiler.current_procinfo.procdef.parast.symtablelevel=labelsym.owner.symtablelevel)) or
              ...
              begin
                if compiler.current_procinfo.procdef.parast.symtablelevel>=labelsym.owner.symtablelevel then
                  begin
    ...
    function tlabelnode.pass_1 : tnode;
      begin
        ...
        if not (nf_internal in flags) then
          include(compiler.current_procinfo.flags,pi_has_label);
        ...
        if (m_non_local_goto in compiler.globals.current_settings.modeswitches) and
          ...
          (compiler.current_procinfo.procdef.parast.symtablelevel<>labsym.owner.symtablelevel) then
...
*****************************************************************************}
    ...
    function ttryexceptnode.pass_1 : tnode;
      begin
        ...
        include(compiler.current_procinfo.flags,pi_uses_exceptions);
