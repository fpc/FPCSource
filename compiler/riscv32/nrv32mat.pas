...
implementation
    ...
    procedure trv32moddivnode.emit_div_reg_reg_reg(signed: boolean; denum, num, res: tregister; ctx:tpassgeneratecodecontext);
      var
        op: TAsmOp;
      begin
        if signed then
    ...
    function trv32moddivnode.first_moddivint: tnode;
      begin
        if (not is_64bitint(resultdef)) and
           (CPURV_HAS_MUL in compiler.target.cpu_capabilities[compiler.globals.current_settings.cputype]) then
          Result:=nil
        else
          result:=inherited;
      end;
