...
****************************************************************************}
    ...
    procedure TNLMexeoutput.createNlm_symbol(const name:shortstring;value:longword;secType:TSecType);
      var
        b:byte;
      begin
        //compiler.verbose.Comment (V_Debug,'TNLMexeoutput.write_symbol '+name);
        {  type (0=.data,1=.code,2,..=????)                   1 byte
           addr                                               4 bytes
           name LString                                     1+n bytes }
        case secType of
  ...
  var
    ...
    alignAmount:longint;
      begin
        with tExeSection(p) do
          begin
            //compiler.verbose.comment (v_debug,'ExeSectionList_write_header: '+name);
            nam := name;
    ...
    procedure TNLMexeoutput.GenerateLibraryImports(ImportLibraryList:TFPHashObjectList);
      var
        ...
        var
          secname:string;
        begin
          //compiler.verbose.Comment (V_Debug,'TNLMexeoutput.GenerateLibraryImports.AddImport '+afuncName);
          result:=nil;
          if assigned(exemap) then
