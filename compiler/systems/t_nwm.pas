...
procedure texportlibnetware.preparelib(AsmData: TAsmData; const s:string);
begin
end;


procedure texportlibnetware.exportprocedure(AsmData: TAsmData; hp : texported_item);
var
  hp2 : texported_item;
begin
...
procedure texportlibnetware.exportvar(AsmData: TAsmData; hp : texported_item);
begin
  hp.is_var:=true;
...
{$ifdef i386}
           ...
           AsmData.asmlists[al_procedures].concat(Tai_symbol.Createname_global(hp2.name^,AT_FUNCTION,0,pd));
...
****************************************************************************}
    ...
    procedure TInternalLinkerNetware.DefaultLinkScript;
      ...
      begin
          ...
          begin
           ...
           { While not SharedLibFiles.Empty do
            ...
            if not hasCopyright then
              if compiler.globals.nwcopyright <> '' then
                Concat('COPYRIGHT "'+compiler.globals.nwCopyright+'"');
        ...
        if not compiler.globals.librarysearchpath.FindFile(s,true,s2) then
          begin
