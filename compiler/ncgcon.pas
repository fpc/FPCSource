         ...
         if not assigned(lab_str) then
           begin
              ...
              if not assigned(entry^.Data) then
                begin
                  datatcb:=ctai_typedconstbuilder.create(ctx.CurrAsmList.AsmData,[tcalo_is_lab,tcalo_make_dead_strippable,tcalo_apply_constalign],compiler);
                   case cst_type of
                      cst_ansistring:
                        begin
                           if len=0 then
...
*****************************************************************************}
    ...
    function tcgsetconstnode.emitvarsetconst(ctx:tpassgeneratecodecontext): tasmsymbol;
      ...
      begin
        ...
        result:=lab;
        lab_set:=lab;
        tcb:=ctai_typedconstbuilder.create(ctx.CurrAsmList.AsmData,[tcalo_is_lab,tcalo_make_dead_strippable,tcalo_apply_constalign],compiler);
