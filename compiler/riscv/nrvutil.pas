...
{$endif defined(RISCV32)}
          if CPURV_HAS_MUL in compiler.target.cpu_capabilities[compiler.globals.current_settings.cputype] then
            attr_arch:=attr_arch+'_m2p0';
          if CPURV_HAS_ATOMIC in compiler.target.cpu_capabilities[compiler.globals.current_settings.cputype] then
            attr_arch:=attr_arch+'_a2p1';
          if CPURV_HAS_F in compiler.target.cpu_capabilities[compiler.globals.current_settings.cputype] then
            attr_arch:=attr_arch+'_f2p2';
          if CPURV_HAS_D in compiler.target.cpu_capabilities[compiler.globals.current_settings.cputype] then
            attr_arch:=attr_arch+'_d2p2';
          if CPURV_HAS_COMPACT in compiler.target.cpu_capabilities[compiler.globals.current_settings.cputype] then
            attr_arch:=attr_arch+'_c2p0';
          if CPURV_HAS_ZICOND in compiler.target.cpu_capabilities[compiler.globals.current_settings.cputype] then
            attr_arch:=attr_arch+'_zicond1p0';
          if CPURV_HAS_CSR_INSTRUCTIONS in compiler.target.cpu_capabilities[compiler.globals.current_settings.cputype] then
            attr_arch:=attr_arch+'_zicsr2p0';
          if CPURV_HAS_FETCH_FENCE in compiler.target.cpu_capabilities[compiler.globals.current_settings.cputype] then
            attr_arch:=attr_arch+'_zifencei2p0';
          if CPURV_HAS_ZIHINTPAUSE in compiler.target.cpu_capabilities[compiler.globals.current_settings.cputype] then
            attr_arch:=attr_arch+'_zihintpause1p0';
          if CPURV_HAS_ZMMUL in compiler.target.cpu_capabilities[compiler.globals.current_settings.cputype] then
            attr_arch:=attr_arch+'_zmmul1p0';
          if CPURV_HAS_ZFA in compiler.target.cpu_capabilities[compiler.globals.current_settings.cputype] then
            attr_arch:=attr_arch+'_zfa1p0';
          if CPURV_HAS_ZBA in compiler.target.cpu_capabilities[compiler.globals.current_settings.cputype] then
            attr_arch:=attr_arch+'_zba1p0';
          if CPURV_HAS_ZBB in compiler.target.cpu_capabilities[compiler.globals.current_settings.cputype] then
            attr_arch:=attr_arch+'_zbb1p0';
          if CPURV_HAS_ZBC in compiler.target.cpu_capabilities[compiler.globals.current_settings.cputype] then
