...
begin
  ...
  if not(compiler.target.info.system in [system_i386_iphonesim,system_arm_ios,system_aarch64_ios,system_x86_64_iphonesim,system_aarch64_iphonesim]) then
    begin
      ...
      if envstr<>'' then
        if not ParseMacVersionMin(compiler.globals.MacOSXVersionMin,iPhoneOSVersionMin,'MAC_OS_X_VERSION_MIN_REQUIRED',envstr,false) then
...
{$endif}
    ...
    begin
      ...
      if envstr<>'' then
        if not ParseMacVersionMin(iPhoneOSVersionMin,compiler.globals.MacOSXVersionMin,'IPHONE_OS_VERSION_MIN_REQUIRED',envstr,true) then
  ...
  case compiler.target.info.system of
    system_powerpc_darwin:
      begin
        if not ParseMacVersionMin(compiler.globals.MacOSXVersionMin,iPhoneOSVersionMin,'MAC_OS_X_VERSION_MIN_REQUIRED','10.3.0',false) then
          internalerror(2022090910);
      end;
    system_powerpc64_darwin:
      begin
        if not ParseMacVersionMin(compiler.globals.MacOSXVersionMin,iPhoneOSVersionMin,'MAC_OS_X_VERSION_MIN_REQUIRED','10.4.0',false) then
          internalerror(2022090911);
      end;
    system_i386_darwin,
    system_x86_64_darwin:
      begin
        if not ParseMacVersionMin(compiler.globals.MacOSXVersionMin,iPhoneOSVersionMin,'MAC_OS_X_VERSION_MIN_REQUIRED','10.8.0',false) then
          internalerror(2022090912);
      end;
    system_arm_ios,
    system_i386_iphonesim:
      begin
        if not ParseMacVersionMin(iPhoneOSVersionMin,compiler.globals.MacOSXVersionMin,'IPHONE_OS_VERSION_MIN_REQUIRED','9.0.0',false) then
    ...
    system_aarch64_iphonesim:
      begin
        if not ParseMacVersionMin(iPhoneOSVersionMin,compiler.globals.MacOSXVersionMin,'IPHONE_OS_VERSION_MIN_REQUIRED','14.0.0',false) then
          internalerror(2023032201);
      end;
    system_aarch64_darwin:
      begin
        if not ParseMacVersionMin(compiler.globals.MacOSXVersionMin,iPhoneOSVersionMin,'MAC_OS_X_VERSION_MIN_REQUIRED','11.0.0',false) then
       ...
       'M':
         begin
           if (compiler.target.info.system in (systems_darwin-[system_i386_iphonesim,system_arm_ios,system_aarch64_ios,system_x86_64_iphonesim,system_aarch64_iphonesim])) and
              ParseMacVersionMin(compiler.globals.MacOSXVersionMin,iPhoneOSVersionMin,'MAC_OS_X_VERSION_MIN_REQUIRED',copy(More,2),false) then
             begin
               break;
             end
           else
       ...
       'P':
         begin
           if (compiler.target.info.system in [system_i386_iphonesim,system_arm_ios,system_aarch64_ios,system_x86_64_iphonesim,system_aarch64_iphonesim]) and
              ParseMacVersionMin(iPhoneOSVersionMin,compiler.globals.MacOSXVersionMin,'IPHONE_OS_VERSION_MIN_REQUIRED',copy(More,2),true) then
             begin
               break;
             end
{$if defined(XTENSA) or defined(RISCV32)}
