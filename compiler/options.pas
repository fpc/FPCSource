   ...
   { now we may show the message or not }
     if show then
      begin
        ...
        HelpLine := PadEnd('',ident)+opt+Copy(s,j+1);
        if HelpLine = '' then
         HelpLineHeight := 1
        else
...
{$ifdef cpucapabilities}
           ...
           if deletepos<>0 then
             begin
               extrasettings:=Copy(s,deletepos);
     ...
     case more[j] of
       ...
       'v' :
         begin
           ...
           if l>0 then
            begin
              val(copy(dllversion,l+1),minor,error);
              if (error=0) and
  ...
  while j<=length(more) do
   begin
     case more[j] of
       ...
       'a' :
         begin
           if not(UpdateAlignmentStr(Copy(Opt,j+3),ParaAlignment)) then
             IllegalPara(opt);
           break;
         end;
       's' :
    ...
    '-' :
      begin
         ...
         case opt[2] of
           ...
           'x' : message1(option_x_ignored,more);
