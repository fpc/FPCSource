            ...
            begin
                  ...
                  if para.left.resultdef.typ=enumdef then
                    begin
                      ...
                      indexpara:=compiler.ccallparanode(
                        compiler.caddrnode_internal(
                          compiler.crttinode(Tenumdef(para.left.resultdef),fullrtti,rdt_normal)
                        ),
                        nil);
                      {Insert a reference to the typinfo.}
                      indexpara:=compiler.ccallparanode(
                        compiler.caddrnode_internal(
                         compiler.crttinode(Tenumdef(para.left.resultdef),fullrtti,rdt_ord2str)
                        ),
                        indexpara);
                ...
                begin
                  ...
                  if para.left.resultdef.typ=enumdef then
                    begin
                      ...
                      indexpara:=compiler.ccallparanode(compiler.caddrnode_internal(
                        compiler.crttinode(Tenumdef(para.left.resultdef),fullrtti,rdt_str2ord)
                      ),nil);
                      {Insert a type conversion to to convert the enum to longint.}
              ...
              if special_handling then
                begin
                  ...
                  addstatement(Tstatementnode(newstatement),
                    compiler.cassignmentnode(para.left,
        ...
        begin
          ...
          if not(do_read) and (para.left.nodetype <> loadn) then
            begin
              ...
              addstatement(Tstatementnode(newstatement),
               compiler.cassignmentnode(compiler.ctemprefnode(temp),
                 para.left));
        ...
        { if we don't have a filepara, create one containing the default }
          ...
          begin
            ...
            addstatement(newstatement,
              compiler.cassignmentnode(compiler.ctemprefnode(filetemp),
                compiler.ccallnode_intern('fpc_get_'+name,nil)));
          ...
          begin
            ...
            if (filepara.left.nodetype <> loadn) then
              begin
                ...
                addstatement(newstatement,
                  compiler.cassignmentnode(compiler.ctemprefnode(filetemp),
                    compiler.caddrnode_internal(filepara.left)));
...
{$endif not cpu64bitaddr}
        ...
        case destpara.resultdef.typ of
          ...
          enumdef:
            begin
              suffix:='enum_';
              sizepara:=compiler.ccallparanode(compiler.caddrnode_internal(
                compiler.crttinode(Tenumdef(destpara.resultdef),fullrtti,rdt_str2ord)
              ),nil);
            end;
          else
        ...
        addstatement(newstatement,compiler.cassignmentnode(
          destpara.left,compiler.ctypeconvnode_internal(tc,destpara.left.resultdef)));
        ...
        if assigned(orgcode) then
          addstatement(newstatement,compiler.cassignmentnode(
              ...
              compiler.ctypeconvnode_internal(
                compiler.ctemprefnode(tempcode),orgcode.resultdef)));
             ...
             orddef:
               begin
                  ...
                  if inlinenumber=in_low_x then
                    ...
                    v:=torddef(def).high;
                  hp:=compiler.cordconstnode(v,def,true);
                  typecheckpass(hp);
                  do_lowhigh:=hp;
               end;
             enumdef:
...
{$r-,q-}
              if shift=0 then
                result:=compiler.cordconstnode(vl2.svalue,def,false)
              else if vl2.svalue<0 then
                result:=compiler.cordconstnode(((vl2.svalue shr shift) or (mask shl (bits-shift))) and mask,def,false)
              else
                result:=compiler.cordconstnode((vl2.svalue shr shift) and mask,def,false);
{$pop}
            end
              ...
              if shift=0 then
                ...
                case inlinenumber of
                  in_ror_x,in_ror_x_y:
                    case def.size of
                      1:
                        result:=compiler.cordconstnode(RorByte(Byte(vl2.svalue),shift),def,false);
                      2:
                        result:=compiler.cordconstnode(RorWord(Word(vl2.svalue),shift),def,false);
                      4:
                        result:=compiler.cordconstnode(RorDWord(DWord(vl2.svalue),shift),def,false);
                      8:
                        result:=compiler.cordconstnode(RorQWord(QWord(vl2.svalue),shift),def,false);
                      else
                  ...
                  in_rol_x,in_rol_x_y:
                    case def.size of
                      1:
                        result:=compiler.cordconstnode(RolByte(Byte(vl2.svalue),shift),def,false);
                      2:
                        result:=compiler.cordconstnode(RolWord(Word(vl2.svalue),shift),def,false);
                      4:
                        result:=compiler.cordconstnode(RolDWord(DWord(vl2.svalue),shift),def,false);
                      8:
                        result:=compiler.cordconstnode(RolQWord(QWord(vl2.svalue),shift),def,false);
                      else
            ...
            if not assigned(left) then
             ...
             begin
               ...
               case inlinenumber of
                 ...
                 in_const_swap_long :
                   hp:=compiler.cordconstnode((vl and $ffff) shl 16+(vl shr 16),left.resultdef,true);
                 in_const_swap_qword :
                   hp:=compiler.cordconstnode((vl and $ffffffff) shl 32+(vl shr 32),left.resultdef,true);
                 in_const_ptr:
                   begin
...
*)
                    ...
                    ordconstn:
                      begin
                        ...
                        if is_integer(left.resultdef) then
                          ...
                          result:=compiler.cordconstnode(vl,left.resultdef,not(nf_internal in flags));
                  ...
                  case left.resultdef.typ of
                    ...
                    arraydef:
                      begin
                        if (inlinenumber=in_low_x) then
                          begin
                            result:=compiler.cordconstnode(int64(tarraydef(
                             left.resultdef).lowrange),tarraydef(left.resultdef).rangedef,true);
                          end
                        else if not is_open_array(left.resultdef) and
                                not is_array_of_const(left.resultdef) and
                                not is_dynamic_array(left.resultdef) then
                          result:=compiler.cordconstnode(int64(tarraydef(left.resultdef).highrange),
                  ...
                  if left.nodetype=ordconstn then
                    begin
                      if tordconstnode(left).value<0 then
                        result:=compiler.cordconstnode((-tordconstnode(left).value),resultdef,false)
                      else
                        result:=compiler.cordconstnode((tordconstnode(left).value),resultdef,false);
                    end
                end;
                  ...
                  if left.nodetype=ordconstn then
                    begin
                      case left.resultdef.size of
                        1:
                          result:=compiler.cordconstnode(BsfByte(Byte(tordconstnode(left).value.uvalue)),resultdef,false);
                        2:
                          result:=compiler.cordconstnode(BsfWord(Word(tordconstnode(left).value.uvalue)),resultdef,false);
                        4:
                          result:=compiler.cordconstnode(BsfDWord(DWord(tordconstnode(left).value.uvalue)),resultdef,false);
                        8:
                          result:=compiler.cordconstnode(BsfQWord(QWord(tordconstnode(left).value.uvalue)),resultdef,false);
                        else
                ...
                begin
                  if left.nodetype=ordconstn then
                    begin
                      case left.resultdef.size of
                        1:
                          result:=compiler.cordconstnode(BsrByte(Byte(tordconstnode(left).value.uvalue)),resultdef,false);
                        2:
                          result:=compiler.cordconstnode(BsrWord(Word(tordconstnode(left).value.uvalue)),resultdef,false);
                        4:
                          result:=compiler.cordconstnode(BsrDWord(DWord(tordconstnode(left).value.uvalue)),resultdef,false);
                        8:
                          result:=compiler.cordconstnode(BsrQWord(QWord(tordconstnode(left).value.uvalue)),resultdef,false);
                        else
              ...
              in_popcnt_x :
                begin
                  if left.nodetype=ordconstn then
                    begin
                      result:=compiler.cordconstnode(PopCnt(tordconstnode(left).value),resultdef,false);
                    end;
                end;
              in_min_single,
              in_min_double:
                      ...
                      if (hp.nodetype=ordconstn) then
                          ...
                          if (hp2.nodetype=ordconstn) then
                            begin
                              ...
                              if inlinenumber=in_min_longint then
                                result:=compiler.cordconstnode(min(LongInt(tordconstnode(hp).value.svalue),LongInt(tordconstnode(hp2).value.svalue)),resultdef,false)
                              else
                                result:=compiler.cordconstnode(min(tordconstnode(hp).value,tordconstnode(hp2).value),resultdef,false);
                            end;

                          if is_minmax_deterministic(tordconstnode(hp), False, helperres) then
                            begin
                              if helperres then
                                result:=compiler.cordconstnode(tordconstnode(hp).value,resultdef,false)
                              else
                                result:=hp2.getcopy();
                            end;
                        end
                        ...
                        begin
                          if is_minmax_deterministic(tordconstnode(hp2), False, helperres) then
                            begin
                              if helperres then
                                result:=compiler.cordconstnode(tordconstnode(hp2).value,resultdef,false)
                              else
                                result:=hp.getcopy();
                            end;
                        end;
                          ...
                          if (hp2.nodetype=ordconstn) then
                            begin
                              ...
                              if inlinenumber=in_max_longint then
                                result:=compiler.cordconstnode(max(LongInt(tordconstnode(hp).value.svalue),LongInt(tordconstnode(hp2).value.svalue)),resultdef,false)
                              else
                                result:=compiler.cordconstnode(max(tordconstnode(hp).value,tordconstnode(hp2).value),resultdef,false);
                            end;
                          ...
                          if (hp2.nodetype=ordconstn) then
                            begin
                              ...
                              if inlinenumber=in_min_dword then
                                result:=compiler.cordconstnode(min(DWord(tordconstnode(hp).value.uvalue),DWord(tordconstnode(hp2).value.uvalue)),resultdef,false)
                              else
                                result:=compiler.cordconstnode(min(tordconstnode(hp).value,tordconstnode(hp2).value),resultdef,false);
                            end;
                          ...
                          if (hp2.nodetype=ordconstn) then
                            begin
                              ...
                              if inlinenumber=in_max_dword then
                                result:=compiler.cordconstnode(max(DWord(tordconstnode(hp).value.uvalue),DWord(tordconstnode(hp2).value.uvalue)),resultdef,false)
                              else
                                result:=compiler.cordconstnode(max(tordconstnode(hp).value,tordconstnode(hp2).value),resultdef,false);
                            end;
          ...
          if (inlinenumber = in_unpack_x_y_z) then
            begin
              ...
              if not is_packed_array(source.left.resultdef) then
                compiler.verbose.CGMessagePos2(source.left.fileinfo,type_e_got_expected_packed_array,'1',source.left.resultdef.typename)
              else
              ...
              if (target.left.resultdef.typ <> arraydef) or
                ...
                compiler.verbose.CGMessagePos2(target.left.fileinfo,type_e_got_expected_unpacked_array,'2',target.left.resultdef.typename)
              else
            ...
            begin
              ...
              if (source.left.resultdef.typ <> arraydef) or
                ...
                compiler.verbose.CGMessagePos2(source.left.fileinfo,type_e_got_expected_unpacked_array,'1',source.left.resultdef.typename)
              else
              ...
              if not is_packed_array(target.left.resultdef) then
                compiler.verbose.CGMessagePos2(target.left.fileinfo,type_e_got_expected_packed_array,'3',target.left.resultdef.typename)
              else
                             ...
                             begin
                               ...
                               if isunaryoverloaded(hp,[]) then
                                 begin
                                   ...
                                   result:=compiler.cassignmentnode(tcallparanode(left).left.getcopy,hp);
                                   exit;
                                 end
                               else
              ...
              in_slice_x:
                begin
                  ...
                  if (resultdef.typ <> arraydef) then
                    ...
                    compiler.verbose.CGMessagePos2(left.fileinfo,type_e_got_expected_unpacked_array,'1',resultdef.typename);
...
{$endif SUPPORT_MMX}
        ...
        if not assigned(result) and not
           compiler.verbose.codegenerror then
          ...
          in_typeinfo_x:
            begin
              if (left.resultdef.typ=enumdef) and
                ...
                result:=compiler.caddrnode_internal(
                  compiler.crttinode(tstoreddef(left.resultdef),fullrtti,rdt_normal)
                );
            end;
          ...
          in_ismanagedtype_x:
            begin
              if left.resultdef.needs_inittable then
                result:=compiler.cordconstnode(1,resultdef,false)
              else
                result:=compiler.cordconstnode(0,resultdef,false);
            end;
       ...
       begin
         ...
         if assigned(tcallparanode(left).right) then
           ...
           begin
             ...
             hpp:=compiler.cordconstnode(1,hdef,false)
           end;
         typecheckpass(hpp);
...
{$endif cpu64bitaddr}
         ...
         addstatement(newstatement,compiler.cassignmentnode(resultnode,hpp));
      ...
      begin
        ...
        if is_ansistring(resultdef) then
          ...
          begin
            ...
            if is_open_array(paradef) then
              begin
                arrayppn:=compiler.caddrnode_internal(ppn.left);
              end
            ...
            rttippn:=compiler.caddrnode_internal(compiler.crttinode(tstoreddef(resultdef),initrtti,rdt_normal));
     ...
     function tinlinenode.first_new: tnode;
       ...
       begin
         ...
         if is_managed_type(tpointerdef(left.resultdef).pointeddef) then
          begin
            para := compiler.ccallparanode(compiler.caddrnode_internal(compiler.crttinode
       ...
       begin
         ...
         if is_shortstring(second) then
           ...
           begin
               ...
               begin
                 if is_const(firstn) then
                   begin
                     ...
                     addstatement(insertstatement,datatemp);
                     datan:=compiler.caddrnode_internal(compiler.ctemprefnode(datatemp));
                   end
                 else
                   begin
                     datan:=firstn;
             ...
             newn:=compiler.ccallparanode(compiler.caddrnode_internal(compiler.crttinode(tstoreddef(second),initrtti,rdt_normal)),
       ...
       begin
         ...
         if is_shortstring(first) then
           ...
           begin
             ...
             newn:=compiler.ccallparanode(compiler.caddrnode_internal
               ...
               begin
                 ...
                 for i:=0 to list.count-1 do
                   begin
                     ...
                     addstatement(newstatement,tempnode);
                     addstatement(newstatement,compiler.cassignmentnode(compiler.ctemprefnode(tempnode),n));
       ...
       begin
         ...
         if assigned(succn) then
           begin
             ...
             addstatement(stmt,compiler.cassignmentnode(compiler.ctemprefnode(tmp),result));
             cmpn:=cmpn.getcopy;
             inserttypeconv_internal(cmpn,resultdef,compiler);
             addstatement(stmt,
               compiler.cassignmentnode(succn,
                 compiler.caddnode(equaln,cmpn,
                   compiler.ctemprefnode(tmp))));
           ...
           begin
             ...
             addstatement(stmt,compiler.cassignmentnode(compiler.ctemprefnode(tmp),n2));
             addstatement(stmt,compiler.ctempdeletenode_normal_temp(tmp));
             addstatement(stmt,compiler.ctemprefnode(tmp));
             result:=n;
           end;
