...
implementation
   ...
   uses
      ...
      psub,
      cgbase,procinfo,
      htypechk,blockutl,pparautl,procdefutil,pass_1,cpuinfo,compiler;
         ...
         begin
           ...
           while assigned(hp) do
            begin
              ...
              if assigned(p4) then
               result:=compiler.caddnode(addn,result,p4);
        ...
        if not(m_tp_procvar in compiler.globals.current_settings.modeswitches) and
          ...
          begin
            ...
            if (fromnode.nodetype=loadn) and
              ...
              tloadnode(fromnode).set_mp(compiler.cloadvmtaddrnode(compiler.ctypenode(tdef(tloadnode(fromnode).symtable.defowner))));
               ...
               begin
                 ...
                 if (arrsize>tstringconstnode(left).len) then
                   begin
                     ...
                     left.free;
                     left:=compiler.cstringconstnode_pchar(pchtemp,arrsize,nil);
                     freemem(pchtemp);
                     typecheckpass(left);
                   end;
                 exit;
        ...
        if left.nodetype=ordconstn then
         ...
         begin
           ...
           { multiply by 10000 for currency. We need to use getcopy to pass
            ...
            begin
              result:=compiler.caddnode(muln,getcopy,compiler.cordconstnode(10000,resultdef,false));
      ...
      begin
        if tstringdef(resultdef).stringtype=st_shortstring then
          begin
            ...
            restemp:=compiler.ctempcreatenode(resultdef,resultdef.size,tt_persistent,false);
            addstatement(newstat,restemp);
            addstatement(newstat,compiler.ccallnode_intern('fpc_pchar_to_shortstr',compiler.ccallparanode(left,compiler.ccallparanode(
              compiler.ctemprefnode(restemp),nil))));
            addstatement(newstat,compiler.ctempdeletenode_normal_temp(restemp));
            addstatement(newstat,compiler.ctemprefnode(restemp));
            result:=newblock;
      ...
      begin
        if tstringdef(resultdef).stringtype=st_shortstring then
          begin
            ...
            restemp:=compiler.ctempcreatenode(resultdef,resultdef.size,tt_persistent,false);
            addstatement(newstat,restemp);
            addstatement(newstat,compiler.ccallnode_intern('fpc_pwidechar_to_shortstr',compiler.ccallparanode(left,compiler.ccallparanode(
              compiler.ctemprefnode(restemp),nil))));
            addstatement(newstat,compiler.ctempdeletenode_normal_temp(restemp));
            addstatement(newstat,compiler.ctemprefnode(restemp));
            result:=newblock;
        ...
        if not(m_tp_procvar in compiler.globals.current_settings.modeswitches) and
          ...
          begin
            ...
            if (left.nodetype=loadn) and
              ...
              tloadnode(left).set_mp(compiler.cloadvmtaddrnode(compiler.ctypenode(tdef(tloadnode(left).symtable.defowner))));
                       ...
                       begin
                         ...
                         if not is_objc_class_or_protocol(resultdef) and
                           ...
                           begin
                             ...
                             addstatement(newstatement,compiler.casnode(left.getcopy,compiler.cloadvmtaddrnode(compiler.ctypenode(resultdef))));
          ...
          niln :
            begin
              ...
              if (resultdef.typ=orddef) then
               begin
                 hp:=compiler.cordconstnode(0,resultdef,true);
                 if ([nf_explicit,nf_internal] * flags <> []) then
          ...
          pointerconstn :
            begin
              ...
              { constant pointer to ordinal }
                ...
                begin
                   hp:=compiler.cordconstnode(TConstExprInt(tpointerconstnode(left).value),
                ...
                case tfloatdef(left.resultdef).floattype of
                  s32real:
                    case tfloatdef(resultdef).floattype of
                      s64real:
                        result:=compiler.ccallnode_intern('stod',compiler.ccallparanode(left,nil));
                      s32real:
                        begin
                          result:=left;
                          left:=nil;
                  ...
                  s64real:
                    case tfloatdef(resultdef).floattype of
                      s32real:
                        result:=compiler.ccallnode_intern('dtos',compiler.ccallparanode(left,nil));
                      s64real:
                        begin
                          result:=left;
                          left:=nil;
      ...
      begin
        ...
        if tstringdef(resultdef).stringtype=st_shortstring then
          begin
            ...
            restemp:=compiler.ctempcreatenode(resultdef,resultdef.size,tt_persistent,false);
            addstatement(newstat,restemp);
            addstatement(newstat,compiler.ccallnode_intern(procname,compiler.ccallparanode(left,compiler.ccallparanode(
              compiler.ctemprefnode(restemp),nil))));
            addstatement(newstat,compiler.ctempdeletenode_normal_temp(restemp));
            addstatement(newstat,compiler.ctemprefnode(restemp));
            result:=newblock;
                ...
                if might_have_sideeffects(left) or
                  ...
                  begin
                    ...
                    addstatement(statement,compiler.caddnode_internal(andn,
                      ...
                      compiler.caddnode_internal(equaln,compiler.cloadvmtaddrnode(compiler.ctemprefnode(tempnode)),right)
                      )
                    );
                  ...
                  begin
                    result:=compiler.caddnode_internal(andn,
                      ...
                      compiler.caddnode_internal(equaln,compiler.cloadvmtaddrnode(left.getcopy),right)
                      );
                    right:=nil;
                  end;
              end
