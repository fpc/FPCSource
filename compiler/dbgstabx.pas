              ...
              if (currfileinfo.fileindex<>0) and
                ...
                begin
                  if curincludefileinfo.fileindex<>0 then
                    begin
                      infile:=compiler.get_module(curincludefileinfo.moduleindex).sourcefiles.get_file(curincludefileinfo.fileindex);
                      list.insertbefore(Tai_stab.Create_str(stabx_ei,'"'+compiler.CFileUtl.FixFileName(infile.name)+'"'),inclinsertpos);
                  ...
                  if currfileinfo.fileindex<>1 then
                    begin
                      ...
                      if assigned(infile) then
                        begin
                          list.insertbefore(Tai_stab.Create_str(stabx_bi,'"'+compiler.CFileUtl.FixFileName(infile.name)+'"'),inclinsertpos);
...
{$endif}
      ...
      if curincludefileinfo.fileindex<>0 then
        begin
          infile:=compiler.get_module(curincludefileinfo.moduleindex).sourcefiles.get_file(curincludefileinfo.fileindex);
          list.insertbefore(Tai_stab.Create_str(stabx_ei,'"'+compiler.CFileUtl.FixFileName(infile.name)+'"'),last);
          curincludefileinfo.fileindex:=0;
        end;
    end;
