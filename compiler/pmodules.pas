...
implementation
    ...
    function TModulesParser.AddUnit(curr : tmodule; const s:string; addasused:boolean = true): tppumodule;
      var
        hp : tppumodule;
        unitsym : tunitsym;
        isnew : boolean;
...
{$endif RISCV32}
{$ifdef AVR}
        if not(curr.is_unit) and (compiler.target.info.system=system_avr_embedded) and
           (cs_link_cvt in compiler.globals.current_settings.globalswitches) then
          if embedded_controllers[compiler.globals.current_settings.controllertype].flashsize>8192 then
...
{$endif DEBUG_NODE_XML}
        ...
        if (compiler.verbose.Errorcount=0) and tppumodule(curr).dependent_module_crc_mismatch then
...
{$endif DEBUG_NODE_XML}
         ...
         { Resource strings }
         GenerateResourceStrings(curr_asmdata,compiler);
