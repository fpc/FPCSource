  ...
  procedure tbasehlcgarm.g_intf_wrapper(list: TAsmList; procdef: tprocdef; const labelname: string; ioffset: longint);
    ...
    procedure loadvmttor12;
      var
        ...
        l : TAsmLabel;
      begin
        reference_reset_base(href,compiler.deftypes.voidpointertype,NR_R0,0,ctempposinvalid,sizeof(pint),[]);
    ...
    procedure op_onr12methodaddr;
      ...
      begin
        ...
        if GenerateThumbCode then
          begin
            reference_reset_base(href,compiler.deftypes.voidpointertype,NR_R0,tobjectdef(procdef.struct).vmtmethodoffset(procdef.extnumber),ctempposinvalid,sizeof(pint),[]);
            if (href.offset in [0..124]) and ((href.offset mod 4)=0) then
              begin
                list.concat(taicpu.op_regset(A_PUSH,R_INTREGISTER,R_SUBWHOLE,[RS_R0]));
          ...
          begin
            reference_reset_base(href,compiler.deftypes.voidpointertype,NR_R12,tobjectdef(procdef.struct).vmtmethodoffset(procdef.extnumber),ctempposinvalid,sizeof(pint),[]);
            cg.a_load_ref_reg(list,OS_ADDR,OS_ADDR,href,NR_R12);
          end;
