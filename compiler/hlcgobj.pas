  ...
  interface
    ...
    type
       ...
       thlcgobjhelpers = class
         ...
         class procedure record_generated_code_for_procdef(AsmData: TAsmData; pd: tprocdef; code, data: TAsmList); virtual;
       end;
...
{$endif cpu64bitalu}
       ...
       public
          {************************************************}
          {                 basic routines                 }
          constructor create(ACompiler: TCompilerBase);overload;virtual;
          constructor create(hlcgobjhelpers: thlcgobjhelpersclass; ACompiler: TCompilerBase);overload;
...
//          procedure location_force_mmreg(list:TAsmList;var l: tlocation;size:tdef;maybeconst:boolean);virtual;abstract;
     ...
     tcreate_hlcgcodegen = procedure(lcgobjhelper: thlcgobjhelpersclass; acompiler: TCompilerBase);
            ...
            if (item<>firstitem) and
              ...
              begin
                if global then
                  ...
                  list.concat(Tai_symbol.Createname_hidden(list.AsmData,item.str,AT_FUNCTION,0,compiler.current_procinfo.procdef));
...
{$endif cpu64bitalu}
  ...
  constructor thlcgobj.create(hlcgobjhelpers: thlcgobjhelpersclass; ACompiler: TCompilerBase);
    begin
    ...
    begin
       ...
       if incrfunc<>'' then
        ...
        begin
          ...
          reference_reset_symbol(href,compiler.RTTIWriter.get_rtti_label(list.AsmData,t,initrtti,def_needs_indirect(t)),0,sizeof(pint),[]);
          if pd.is_pushleftright then
            begin
              a_loadaddr_ref_cgpara(list,t,ref,cgpara1);
  ...
  procedure thlcgobj.g_array_rtti_helper(list: TAsmList; t: tdef; const ref: treference; const highloc: tlocation; const name: string);
    ...
    begin
      ...
      if highloc.loc=LOC_CONSTANT then
        a_load_const_cgpara(list,compiler.deftypes.sizesinttype,highloc.value+1,cgpara3)
      else
        begin
          if highloc.loc in [LOC_REGISTER,LOC_CREGISTER] then
            ...
            begin
              hreg:=getintregister(list,compiler.deftypes.sizesinttype);
              a_load_loc_reg(list,compiler.deftypes.sizesinttype,compiler.deftypes.sizesinttype,highloc,hreg);
            end;
          { increment, converts high(x) to length(x) }
          lenreg:=getintregister(list,compiler.deftypes.sizesinttype);
          a_op_const_reg_reg(list,OP_ADD,compiler.deftypes.sizesinttype,1,hreg,lenreg);
          a_load_reg_cgpara(list,compiler.deftypes.sizesinttype,lenreg,cgpara3);
        end;
    ...
    begin
      ...
      if pd.is_pushleftright then
        begin
          ...
          { load size }
          a_load_reg_cgpara(list,compiler.deftypes.sizesinttype,sizereg,cgpara3);
        end
      else
        begin
          { load size }
...
{$endif cpu64bitalu}
  ...
  procedure thlcgobj.gen_initialize_code(list: TAsmList);
    begin
      ...
      if (compiler.current_procinfo.procdef.proctypeoption<>potype_exceptfilter) or
          ...
          begin
            ...
            if (compiler.current_procinfo.procdef.proctypeoption=potype_exceptfilter) then
              begin
                ...
                if href.base<>compiler.current_procinfo.framepointer then
                  begin
                    hp:=hp^.next;
                    continue;
                  end;
              ...
              begin
                if paramanager.push_high_param(tparavarsym(p).varspez,tparavarsym(p).vardef,compiler.current_procinfo.procdef.proccalloption) then
                  begin
                    ...
                    if assigned(compiler.current_procinfo.finalize_procinfo) and
                       (compiler.current_procinfo.finalize_procinfo.procdef.proctypeoption=potype_exceptfilter) and
...
//                      cg.g_copyvaluepara_packedopenarray(list,href,hsym.intialloc,tarraydef(tparavarsym(p).vardef).elepackedbitsize,hreg);
  ...
  procedure thlcgobj.record_generated_code_for_procdef(AsmData: TAsmData; pd: tprocdef; code, data: TAsmList);
    begin
      fphlcgobjhelper.record_generated_code_for_procdef(AsmData, pd, code, data);
    end;
