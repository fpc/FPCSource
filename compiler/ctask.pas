...
type
  ...
  ttask = class
    ...
    state : tglobalstate;
    prev, next: ttask;
    constructor create(m : tmodule; acompiler: TCompilerBase);
...
constructor ttask.create(m: tmodule; acompiler: TCompilerBase);
begin
  inherited create;
  compiler:=acompiler;
  module:=m;
...
begin
  Result:=ttask.create(m,compiler);
...
function ttask_handler.check_compiled_uses(dst: tmodule): tmodule;
  ...
  begin
    visited:=nil;
    SetLength(visited,length(tcompiler(compiler).all_modules));
