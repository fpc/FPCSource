...
implementation
  ...
  procedure thlcgaarch64.makeregssamesize(list: tasmlist; fromsize, tosize: tcgsize; orgfromreg, orgtoreg: tregister; out newfromreg, newtoreg: tregister);
    begin
  ...
  procedure thlcgaarch64.g_intf_wrapper(list: TAsmList; procdef: tprocdef; const labelname: string; ioffset: longint);
    ...
    begin
      ...
      make_global:=false;
      if (not compiler.current_module.is_unit) or compiler.globals.create_smartlink_library or
      ...
      if (po_virtualmethod in procdef.procoptions) and
        ...
        begin
          ...
          a_load_ref_reg(list,compiler.deftypes.voidpointertype,compiler.deftypes.voidpointertype,href,NR_IP0);
          { jmp *vmtoffs(%eax) ; method offs }
          reference_reset_base(href,compiler.deftypes.voidpointertype,NR_IP0,tobjectdef(procdef.struct).vmtmethodoffset(procdef.extnumber),ctempposinvalid,sizeof(pint),[]);
          op:=A_LDR;
  ...
  procedure create_hlcodegen_cpu(hlcgobjhelpers: thlcgobjhelpersclass; compiler: TCompilerBase);
    begin
      tcompiler(compiler).hlcg:=thlcgaarch64.create(hlcgobjhelpers,compiler);
    end;
