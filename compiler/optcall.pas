  ...
  implementation
    ...
    function TCallOptimizer.doinline(var _n: tnode; arg: pointer): foreachnoderesult;
      ...
      begin
        ...
        if not(callnode.doinlining) then
          begin
            if not(po_compilerproc in callnode.procdefinition.procoptions) then
              compiler.verbose.Message1(cg_n_no_inline,tprocdef(callnode.procdefinition).customprocname([pno_proctypeoption, pno_paranames,pno_ownername, pno_noclassmarker, pno_prettynames]));
            exit;
          end;
        ...
        inlineblock:=internalstatements(compiler,callnode.inlineinitstatement);
        ...
        inlinecleanupblock:=internalstatements(compiler,callnode.inlinecleanupstatement);
        ...
        if (cnf_return_value_used in callnode.callnodeflags) and
           not(equal_defs(compiler.symtablestack,_n.resultdef,callnode.resultdef)) then
