    ...
    function TObjectiveCUtils.objcloadbasefield(n: tnode; const fieldname: string): tnode;
      ...
      begin
        ...
        if fieldname='ISA' then
          ...
          begin
            result:=compiler.cderefnode(compiler.ctypeconvnode_internal(n,compiler.deftypes.objc_idtype));
    ...
    function TObjectiveCUtils.objcsuperclassnode(def: tdef): tnode;
      ...
      begin
        ...
        if is_objcclassref(def) then
            ...
            if (oo_is_classhelper in tobjectdef(tclassrefdef(def).pointeddef).objectoptions) then
              begin
                ...
                if (compiler.target.info.system in systems_objc_nfabi) and
                   (not compiler.target.MacOSXVersionMin.isvalid or
              ...
              begin
                ...
                para:=compiler.ccallparanode(compiler.cstringconstnode_str(tobjectdef(tclassrefdef(def).pointeddef).objextname^),nil);
          ...
          begin
            if not(oo_is_classhelper in tobjectdef(def).objectoptions) then
              result:=compiler.cloadvmtaddrnode(compiler.ctypenode(def))
            else
              result:=compiler.cloadvmtaddrnode(compiler.ctypenode(tobjectdef(def).childof));
            tloadvmtaddrnode(result).forcall:=true;
          end;
        ...
        if not(compiler.target.info.system in systems_objc_nfabi) or
           (compiler.target.MacOSXVersionMin.isvalid and
            (compiler.target.MacOSXVersionMin.relationto(10,6,0)<0)) then
