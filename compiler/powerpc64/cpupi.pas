...
begin
  ...
  if not (po_assembler in procdef.procoptions) then begin
    ...
    if (cs_profile in compiler.globals.init_settings.moduleswitches) and (ofs < minstacksize) then begin
      ofs := minstacksize;
    end;
    tg.setfirsttemp(ofs);
  end else begin
...
begin
  ...
  if not (po_assembler in procdef.procoptions) then begin
    ...
    if (pi_do_call in flags) or (tg.lasttemp <> tg.firsttemp) or
      (result > RED_ZONE_SIZE) {or (cs_profile in compiler.globals.init_settings.moduleswitches)} then begin
