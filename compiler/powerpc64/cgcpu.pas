  ...
  if (compiler.target.info.system <> system_powerpc64_darwin) then
    ...
    rg[R_INTREGISTER] := trgintcpu.create(R_INTREGISTER, R_SUBWHOLE,
      [{$ifdef user0} RS_R0, {$endif} RS_R3, RS_R4, RS_R5, RS_R6, RS_R7, RS_R8,
       ...
       RS_R14], first_int_imreg, [],compiler)
  else
...
//  if (((cs_opt_peephole in compiler.globals.current_settings.optimizerswitches) or (cs_create_pic in compiler.globals.current_settings.moduleswitches)) and
...
var
  ...
  begin
    if (a = 0) then begin
      ...
      list.concat(taicpu.op_reg_reg(negops[cs_check_overflow in compiler.globals.current_settings.localswitches], dst, src));
...
var
  ...
  begin
    { there are two ways to do this: manually, by generating a few "ld" instructions,
     or via the restore helper functions. The latter are selected by the -Og switch,
     i.e. "optimize for size" }
    if (cs_opt_size in compiler.globals.current_settings.optimizerswitches) then begin
