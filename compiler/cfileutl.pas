      ...
      begin
        if not TryUseCache then
          begin
            ...
            result:=FOwnerFileUtils.FileExists(Name+AName,false);
            exit;
          end;
...
{
   function TCompilerFileUtils.FindFilePchar(const f : TCmdStr;path : pchar;allowcache:boolean;var foundfile:TCmdStr):boolean;
      Var
  ...
  function  TCompilerFileUtils.FindFileInExeLocations(const bin:TCmdStr;allowcache:boolean;var foundfile:TCmdStr):boolean;
    var
      ...
      found : boolean;
    begin
       found:=FindFile(FixFileName(bin),TReadOnlyCompilerGlobals(Globals).exepath,allowcache,foundfile);
      if not found then
       begin
{$ifdef macos}
