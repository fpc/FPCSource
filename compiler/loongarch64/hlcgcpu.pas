...
implementation
  ...
  procedure thlcgloongarch64.g_intf_wrapper(list: TAsmList; procdef: tprocdef; const labelname: string; ioffset: longint);
    ...
    begin
      ...
      if make_global then
        list.concat(Tai_symbol.Createname_global(list.AsmData,labelname,AT_FUNCTION,0,compiler.deftypes.voidcodepointertype))
      else
        list.concat(Tai_symbol.Createname_hidden(list.AsmData,labelname,AT_FUNCTION,0,compiler.deftypes.voidcodepointertype));
  ...
  procedure create_hlcodegen_cpu(hlcgobjhelpers: thlcgobjhelpersclass; compiler: TCompilerBase);
    begin
      tcompiler(compiler).hlcg:=thlcgloongarch64.create(hlcgobjhelpers,compiler);
    end;
