...
begin
  ...
  Try
    If not {$IFDEF FPC_DOTTEDUNITS}WinApi.Jedi.{$ENDIF}WinSvc.StartService(SHandle,Argc,PChar(PArgs)) then
      RaiseLastOSError;
  Finally
    If (PArgs<>Nil) then
      FreePCharList(PArgs);
...
begin
  FDBLock:={$IFDEF FPC_DOTTEDUNITS}WinApi.Jedi.{$ENDIF}WinSvc.LockServiceDatabase(Handle);
  If FDBLock=Nil then
    RaiseLastOSError;
end;
...
begin
  If (FDBLock<>Nil) then
    begin
    Try
      If Not {$IFDEF FPC_DOTTEDUNITS}WinApi.Jedi.{$ENDIF}WinSvc.UnLockServiceDatabase(FDBLock) then
        RaiseLastOSError;
    Finally
      FDBLock:=Nil;
    end;
...
begin
  {$IFDEF FPC_DOTTEDUNITS}WinApi.Jedi.{$ENDIF}WinSvc.QueryServiceConfig(SHandle,Nil,0,BytesNeeded);
  If (GetLastError<>ERROR_INSUFFICIENT_BUFFER) then
    RaiseLastOSError;
  GetMem(SvcCfg,BytesNeeded);
  Try
    If Not {$IFDEF FPC_DOTTEDUNITS}WinApi.Jedi.{$ENDIF}WinSvc.QueryServiceConfig(SHandle,SvcCfg,BytesNeeded,BytesNeeded) then
      RaiseLastOSError;
