...
{$IFDEF GO32V2}
BEGIN
BytesMoved:=system.do_read(Handle,longint(@BufferArea),BufferLength);
DosRead:=InOutRes;
End;
{$ELSE not GO32V2}
BEGIN
...
{$IFDEF GO32V2}
BEGIN
system.do_write(Handle,longint(@BufferArea),BufferLength);
...

{
  ...
  * Restored version
  ...
  Revision 1.5  1998/01/26 11:56:46  michael
  + Added log at the end


  
  Working file: rtl/dos/objects.pp
  description:
  ----------------------------
  revision 1.4
  date: 1998/01/09 16:05:41;  author: florian;  state: Exp;  lines: +1 -3
    + ojbpash.inc and objpas.inc
    * {$E-} from objects.pp removed to avoid a warning
  ----------------------------
  revision 1.3
  date: 1998/01/06 00:29:29;  author: michael;  state: Exp;  lines: +3 -3
  Implemented a system independent sequence of reset/rewrite/append fileopenfunc etc system \n (from Peter Vreman)
  ----------------------------
  revision 1.2
  date: 1997/12/01 12:15:47;  author: michael;  state: Exp;  lines: +13 -15
